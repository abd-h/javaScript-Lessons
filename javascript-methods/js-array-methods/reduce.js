...
of all array operations in JavaScript. You can solve almost any array 
processing problem using the reduce method.

The reduce method allows for more general forms of array processing, 
and it's possible to show that both filter and map can be 
derived as special applications of reduce. The reduce method iterates 
over each item in an array and returns a single value (i.e. string, number, object, array). 
...
directed by Christopher Nolan. Recall from prior challenges how to 
filter data and map over it to pull what you need. 
You may need to create other variables, 
...
function getRating(watchList) {
    ...
    let number = nolanMovies.map(number => parseFloat(number.imdbRating));
    let ratingSum = number.reduce((sum, sumRating) => (sum + sumRating), 0)


    let avarageRAting = ratingSum / count;
    //code below works as well.
    // let avarageRAting = watchList
                        // .filter(director => director.Director === "Christopher Nolan")
                        // .map(number => Number(number.imdbRating))                       
                        // .reduce((sum, sumOfRating) => (sum  + sumOfRating)) /
...
/*Use Higher-Order Functions map, filter, or reduce to 
...
complete the code for the squareList function using 
any combination of map(), filter(), and reduce() so that it
 returns a new array containing only the square of only 
...
const squareList = arr => {
    return arr
                    .filter(integer => integer >= 0 && integer === parseInt(integer))
...
// returns  [25, 9];
const squaredInteger = [2, 3, 4, 5]
let n = squaredInteger.reduce(function (integer, total) {
