    ...
    */
    ...
    const newUsers1 = newUser1.map(item => ({
      name:item.name,
        age: item.age
    }));
...
console.log(Array.prototype.myMap());
    ...
    This means that it returns a new array that contains 
    an image of each element of the array. 
    It will always return the same number of items. */

    const myAwsomeArray = [5,4,3,2,1];
    const mySqr = myAwsomeArray.map(x => x * x);
    console.log(mySqr);

    /**Like map , the forEach() method receives a function as an argument 
     * and executes it once for each array element. 
    ...
    let ab = [];
    const newObj = myObjArr.forEach(element => 
       console.log(element.name, element.id)
    //    ab.push(element.name + " " + element.id) do this to get return
...
The forEach() method returns undefined and map() returns 
...
const feArr = forEachArr.forEach(x => x * x);
console.log(feArr);// returns undefined

const mapArr = forEachArr.map(x => x * x);
console.log(mapArr); // returns [1, 4, 9, 16, 25]


/**2. Ability to chain other methods
The second difference between these array methods is the fact 
that map() is chainable. This means that you can attach 
reduce(), sort(), filter() and so on after performing a map() 
method on an array.

That's something you can't do with forEach() because, 
