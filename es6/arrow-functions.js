    ...
    */
    ...
    console.log(maxi); // returns 89

    /*We had to use Math.max.apply(null, arr) because Math.max(arr) returns 
    NaN. Math.max() expects comma-separated arguments, but not an array. 
    The spread operator makes this syntax much better to read and maintain.*/

    const myArr = [6,89,3,45,99];
    const myMaxi = Math.max(...myArr);
    console.log(myMaxi);
    /*maximus would have a value of 99.
